...
| `proc:bifora` | two-light window (July 2026, Giotto's-campanile grammar): twin 4-facet arched lights split by a colonnette (base + capital), springing at 0.30 so their apexes poke into the lunette of the grander 8-facet voussoir arch (0.22 opening, band 0.065 — sized so `biforaWindow`'s full-lunette reveal corners stay inside the *faceted* ring), octagonal roundel boss in the spandrel; the inner order recesses a hair behind the frame planes | palazzo piano nobile ×4/face + sides (`biforaWindow` — reveal + frame, **no glazing**: the lights stay dark voids), bell tower all 4 storeys (0.95, `verdeLight`), cathedral west front (2 per aisle, `verdeLight`; the nave pair gave way to the rose) |
| `proc:arch-leaf` | arched glazed leaf (July 2026, diamond-lattice reference): glass filling the whole arch to the lunette apex — ±45° leaded bars sample-clipped to the arch outline (`LATTICE_PITCH` 0.04, kept chunky to match the kit's detail level), centre mullion + transoms + springline rail, wood ring following the glass arc | every `archWindow` (replaced the rect `proc:shutter` there, which left the lunette dark reveal) |
| `proc:rose` + `proc:rose-glass` | rose window (July 2026, SMN reference — despite the batch-2 plan, passable in code after all): 16-facet stone annulus with voussoir-alternating shades + recessed darker inner step, over a glazing disc with wheel tracery (12 spokes off a hub through two concentric rings; sector sparkle muted to 40%). The one **CENTERED** piece pair (position = circle centre, not base-at-0): `roseWindow()`'s `sy` param pre-stretches y against a prefab's `scaleY` squash (cathedral: 1/0.71 — exact, since stretch-fit `scaleY = min(scaleX, scaleZ) × 0.71`), and centred pieces scale about the circle so the stretch keeps them round | cathedral west front (0.9, `verdeLight` ring), centered over the grand portal in the slot the paired nave bifore held |
...
|---|---|---|
| ~~**2**~~ | ~~rose window~~ generated too (July 2026 — see `proc:rose` in the fittings table; a hand-modeled swap stays drop-in, same envelope) | — |
