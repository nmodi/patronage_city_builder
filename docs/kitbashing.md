...
Nine pieces are built in code (`render/proceduralPieces.ts`) instead of loaded: **`proc:block`**, **`proc:roof-gable`**, **`proc:gable-end`**, **`proc:roof-hip`**, plus the batch-1 fittings (July 2026): **`proc:surround-rect`**, **`proc:surround-arch`**, **`proc:door-frame`**, **`proc:door-leaf`**, **`proc:arch-bay`** — the artist-brief pieces, generated to the brief's own specs (pietra serena `stone` + `wood` flat materials; arches are 6–8 flat wedge facets with alternating vertex shades as voussoir joints). They enter through the ordinary file path — `getContainer` branches on the `proc:` prefix and everything downstream (material conversion, `MATERIAL_TINTS`, tints, desaturation, batching, blend stretch) treats them as untextured Nature-Kit-style files. Each fills the envelope of the kit piece it replaces, because the manifest's positions and `fit` values are tuned to those bounds — exactly for the block, and *core*-only for the roof, whose tiles stand proud of it (`ROOF_TILE_BULGE`). `proceduralPieces.check.ts` enforces both.

**For what's still open** — the artist commission for the organic pieces ([artist-brief.md](artist-brief.md)); the panel-to-fitting migration completed July 2026 — see [procedural-pieces.md](procedural-pieces.md). This section is the field notes; that one is the plan.
...
- **`proc:block` must match the kit's stucco average, not merely look nice** — it was pinned against the kit panels' ramp while they still stood beside it, and it is now the city's established wall colour (the check pins it). Drift it and every already-tuned tint and texture shifts with it.
...
**Still flat:** nothing — `proc:roof-hip` and the gable family have since taken every kit roof (only the obelisk's stone cap still uses `roof-point`, and it is not a roof). See procedural-pieces.md's built table.
...
- `wall-arch`, `wall-door`, `wall-window-*`, `banner-*` are **thin panels on the +X face** of a unit cell (x ≈ 0.4→0.5). Pick the face with `rotationY` 0 / π / ±π/2. *(The wall panels are at 0 refs since the panel-free pass, July 2026 — every opening is a generated fitting now; `banner-*` are props and stay. `wall-door`/`wall-window-shutters` remain on disk as `make-plain-openings.py` inputs.)*
...
- `wall-arch` is a flat pier strip (z 0.2–0.5 only), **not** an actual arch. The kit has no dome or curved-arch piece — `proc:arch-bay` is the real arch now, and `wall-arch` is at 0 refs.
...
- Per-building tints: `Part.tint` ("facade"/"roof" resolve from category palettes by position hash; other strings index `TINT_COLORS` directly, e.g. religious "verde"). The tint is a diffuse multiply on a cloned material pair and joins the batch mesh key. **One tint per part, applied to every material it has** — a detail that must move independently of its wall needs to be its own *part* (see `proc:gable-end`), not its own material. Don't reach for a per-material tint record: `buildHosts` parses the tint back out of the mesh key and applies it to every mesh of the file, so per-material tinting means reworking the host builder too.
- Accents baked into the atlas that a whole-material multiply can't isolate use a colormap **texture variant** instead (`TEXTURE_TINTS` — today the market-stall awning cloths, `make-stall-cloth.py`). The religious "mint" variant retired with the panel-free pass (July 2026): every opening is a generated fitting, so religious green is plain `Part.tint: "verde"` on proc pieces (the chapel's oculi), and `make-mint-quoins.py` + `colormap-mint*.png` are deleted.
- **The kit's door/window/arch panels were full-face slabs** carrying the atlas's baked quoins — one salmon bar per panelled corner, z-fighting where two perpendicular panels claimed the same wedge. Retired (July 2026): every opening across the roster is now a generated fitting (`windowOn`/`doorOn`/`archWindow`/`portalOn` in the manifest), which ended the corner-quoin flicker for good. See procedural-pieces.md §1.
