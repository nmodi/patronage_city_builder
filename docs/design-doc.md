...
- **Diagonal (45°) placement** *(built — July 2026; see Roads → Snap-to-road placement + 45° buildings)*. Open questions resolved: diagonal footprints claim a true cell mask (not the bounding box), plaza connectivity flows through mask cells unchanged, and row-house blending / colonnade extend are skipped at 45° in v1. Remaining follow-ups: diagonal row-house blending (a diagonal terrace along a diagonal street), diagonal drags for Dirt Path / Stone Bridge, and junction autotiling.
